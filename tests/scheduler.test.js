...
function teste(extra) {
  return Object.assign({
    id: 'TP-01', nome: 'Ensaio', area: 'COLD', equipamentoId: 'EQ-01', fases: ['DV'],
...
function peca(extra) {
  return Object.assign({
    id: 'PC-01', nome: 'Peça', clienteId: 'CLI-01', area: 'COLD',
    dataAmostras: SEGUNDA, quantidade: 10, custoAmostra: 500
  }, extra);
}
...
test('nenhum ensaio começa antes da chegada das amostras', () => {
  const s = estado({ pecas: [peca({ dataAmostras: '2026-08-10' })] });
...
test('amostra já disponível no passado não puxa o ensaio para trás', () => {
  const s = estado({ pecas: [peca({ dataAmostras: '2026-01-05' })] });
  const plano = scheduler.planejar(s, SEGUNDA);
...
test('a janela atravessa o fim de semana quando o equipamento só opera em dias úteis', () => {
  ...
  const s = estado({
    ...
    pecas: [peca({ dataAmostras: '2026-07-09' })]
  });
  const plano = scheduler.planejar(s, SEGUNDA);
  const a = alocacaoDe(plano, 'DM-01');
...
test('o catálogo de exemplo é planejável de ponta a ponta', () => {
  ...
  base.demandas = base.testes.slice(0, 12).map((t, i) => {
    const compativel = base.pecas.find((p) => t.area === 'AMBOS' || p.area === t.area);
    return demanda({
      id: 'D' + i, testeId: t.id, pecaId: compativel.id, clienteId: compativel.clienteId,
      prioridade: i % 3 === 0 ? 'ALTA' : 'MEDIA'
    });
  });
  const plano = scheduler.planejar(base, SEGUNDA);
  assert.equal(plano.bloqueadas.length, 0);
  assert.equal(plano.agendadas.length, 12);
  plano.agendadas.forEach((a) => {
    assert.ok(util.diffDias(a.peca.dataAmostras, a.inicio) >= 0, a.teste.id + ' começa antes da amostra');
